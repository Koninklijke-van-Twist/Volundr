 ...
 * (via odata_get_all / company-discovery; nightly/hourly doen vandaag géén BC-OData)
 * naar Mímir i.p.v. BC.
 *
 * Met $mimirApi gezet zijn $auth_list / $environment / $baseUrl / $auth ongebruikt voor BC;
 * Mímir beheert environments — Völundr heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
 ...
 *
 * max_age-beleid:
 * - nightly.php: weekrapport uit cache — VOLUNDR_NIGHTLY_MAX_AGE (14400) gereserveerd
 * - hourly.php: 101Connect-refresh (geen BC-OData) — VOLUNDR_HOURLY_MAX_AGE (1800) gereserveerd
 * - UI / on-demand: bestaande TTL (odata_get_all-default / VOLUNDR_ODATA_TTL = 300)
 */

/** Mímir max_age for nightly-style BC fetches (4h). nightly.php doet vandaag geen BC-OData. */
const VOLUNDR_NIGHTLY_MAX_AGE = 14400;

/** Mímir max_age for hourly-style OData fetches (≤30 min). hourly.php haalt 101Connect, geen BC. */
const VOLUNDR_HOURLY_MAX_AGE = 1800;

/** Mímir max_age for UI / on-demand (bestaande odata_get_all-default). */
const VOLUNDR_ODATA_TTL = 300;
...
function odata_mimir_request(string $method, string $path, ?array $jsonBody = null): array
{
    ...
    $opts = [
        ...
        CURLOPT_USERAGENT => 'Volundr-MimirClient/1.0',
    ];
    if ($jsonBody !== null) {
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_api_key() !== '') {
        // Mímir beheert de BC-cache (max_age); Völundr-filecache / live-paginering worden overgeslagen.
        return odata_mimir_fetch_all($url, $ttlSeconds === 0 ? VOLUNDR_ODATA_TTL : $ttlSeconds);
    }
...
{
    $ch = curl_init($url);
    $userAgent = 'Volundr-ODataClient/1.0 (Windows; nl-NL)';
